...
theorem recF_eq_of_wEquiv (α : TypeVec n) {β : Type u} (u : F (α ::: β) → β) (x y : q.P.W α) :
...
theorem wEquiv.abs' {α : TypeVec n} (x y : q.P.W α)
    (h : QPF.abs (q.P.wDest' x) = QPF.abs (q.P.wDest' y)) :
    WEquiv x y := by
  revert h
  apply q.P.w_cases _ x
  intro a₀ f'₀ f₀
...
theorem wEquiv.symm {α : TypeVec n} (x y : q.P.W α) : WEquiv x y → WEquiv y x := by
  ...
  | trans x y z _e₁ _e₂ ih₁ ih₂ => exact QPF.WEquiv.trans _ _ _ ih₂ ih₁
...
theorem wrepr_wMk {α : TypeVec n} (a : q.P.A) (f' : q.P.drop.B a ⟹ α)
    ...
    wrepr (q.P.wMk a f' f) =
      q.P.wMk' (repr (abs (q.P.map (id ::: wrepr) ⟨a, q.P.appendContents f' f⟩))) := by
...
theorem wEquiv_map {α β : TypeVec n} (g : α ⟹ β) (x y : q.P.W α) :
    WEquiv x y → WEquiv (q.P.wMap g x) (q.P.wMap g y) := by
  intro h; induction h with
...
```
-/
def Fix {n : Nat} (F : TypeVec (n + 1) → Type _) [q : QPF F] (α : TypeVec n) :=
  Quotient (wSetoid α : Setoid (q.P.W α))

/-- `Fix F` is a functor -/
def Fix.map {α β : TypeVec n} (g : α ⟹ β) : Fix F α → Fix F β :=
  Quotient.lift (fun x : q.P.W α => Quotient.mk (wSetoid β) (q.P.wMap g x))
    fun _a _b h => Quot.sound (wEquiv_map _ _ _ h)
...
theorem Fix.ind_aux (a : q.P.A) (f' : q.P.drop.B a ⟹ α) (f : q.P.last.B a → q.P.W α) :
    Fix.mk (abs ⟨a, q.P.appendContents f' (fun x => Quotient.mk (wSetoid α) (f x))⟩) =
      Quotient.mk (wSetoid α) (q.P.wMk a f' f) := by
...
theorem Fix.ind_rec {β : Type u} (g₁ g₂ : Fix F α → β)
    (h : ∀ x : F (append1 α (Fix F α)),
        (id ::: g₁) <$$> x = (id ::: g₂) <$$> x → g₁ (Fix.mk x) = g₂ (Fix.mk x)) :
    ∀ x, g₁ x = g₂ x := by
  apply Quot.ind
  intro x
  apply q.P.w_ind _ x
...
theorem Fix.mk_dest (x : Fix F α) : Fix.mk (Fix.dest x) = x := by
  ...
  intro h; rw [h]
  change Fix.mk ((id ::: id) <$$> x) = Fix.mk x
...
theorem Fix.dest_mk (x : F (append1 α (Fix F α))) : Fix.dest (Fix.mk x) = x := by
  ...
  have : Fix.mk ∘ @Fix.dest n F q α = _root_.id := by
    ext (x : Fix F α)
    apply Fix.mk_dest
  rw [this, appendFun_id_id]

theorem Fix.ind {α : TypeVec n} (p : Fix F α → Prop)
    (h : ∀ x : F (α ::: (Fix F α)), LiftP (PredLast α p) x → p (Fix.mk x)) : ∀ x, p x := by
  apply Quot.ind
  intro x
  apply q.P.w_ind _ x; intro a f' f ih
  change p (Quotient.mk (wSetoid α) (q.P.wMk a f' f))
  rw [← Fix.ind_aux a f' f]
  apply h
  rw [QPF.liftP_iff]
  refine ⟨_, _, rfl, ?_⟩
...
instance qpfFix : QPF (Fix F) where
  P := q.P.wp
