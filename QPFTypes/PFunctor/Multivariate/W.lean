...
inductive WPath : P.last.W → Fin n → Type u
  | root (a : P.A) (f : P.last.B a → P.last.W) (i : Fin n) (c : P.drop.B a i) : WPath ⟨a, f⟩ i
...
/--
`P.W` is the W-type of `P`.

This is just an abbreviation of `P.wp.Obj`
-/
...
/--
`P.wMap g` maps an n-ary function `g : α ⟹ β`
over objects of the W-type `P.W α`.
...
theorem w_map_wMk {α β : TypeVec n} (g : α ⟹ β) (a : P.A) (f' : P.drop.B a ⟹ α)
    (f : P.last.B a → P.W α) :
    P.wMap g (P.wMk a f' f) = P.wMk a (g ⊚ f') fun i => P.wMap g (f i) := by
  change _ = P.wMk a (g ⊚ f') (P.wMap g ∘ f)
  have : P.wMap g ∘ f = fun i => ⟨(f i).fst, g ⊚ (f i).snd⟩ := by
    ext i : 1
