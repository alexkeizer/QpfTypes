...
def Fix.drec {F : TypeVec.{u} (n + 1) → Type u} {β : Fix F α → Sort u}
    (g :
      ∀ x : F (α ::: PSigma β),
        β (Fix.mk <| (id ::: PSigma.fst) <$$> x)) (x : Fix F α) : β x :=
  let y := @Fix.rec _ F _ _ α (PSigma β) (fun i => ⟨_, g i⟩) x
  have : x = y.1 := by
    symm
    dsimp [y]
    ...
    rw [Fix.rec_eq]
    dsimp only [id_eq]
    simp? [appendFun_id_id] at ih says -- why `says` seems unnecessary
