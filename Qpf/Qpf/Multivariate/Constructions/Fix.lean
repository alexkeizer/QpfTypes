...
variable {n : ℕ}
variable {F : TypeFun.{u} (n + 1)} [q : MvQpf F]
...
def wSetoid (α : TypeVec n) : Setoid (q.P.W α) :=
  ⟨Wequiv, @Wequiv.refl _ _ _ _, @Wequiv.symm _ _ _ _, @Wequiv.trans _ _ _ _⟩
...
theorem Fix.dest_mk (x : F (append1 α (Fix F α))) : Fix.dest (Fix.mk x) = x := by
  ...
  have : Fix.mk ∘ (@Fix.dest _ F _ α) = _root_.id := by
    ext x
    apply Fix.mk_dest
...
def Fix.drec {β : Fix F α → Type u} 
  ...
  have hy : y = @Fix.rec _ F _ α (Sigma β) (fun i => ⟨_, g i⟩) x
    := by trivial
  have : x = y.1 := 
  by
    apply Eq.symm
