...
* `AddConst α` is `Const α` but for when `α` has an additive structure.
...
theorem Functor.ext {F} :
    ∀ {F1 : Functor F} {F2 : Functor F} [@LawfulFunctor F F1] [@LawfulFunctor F F2] H :
...
instance instFunctor {γ} : Functor (Const γ) where
  map := @Const.map γ

instance instLawfulFunctor {γ} : LawfulFunctor (Const γ) := by
...
instance AddConst.instLawfulFunctorConstInstFunctorConst {γ} : LawfulFunctor (AddConst γ) :=
  @Const.instLawfulFunctor γ
...
protected theorem id_map : ∀ x : Comp F G α, Comp.map id x = x
  | Comp.mk x => by
    simp [Comp.map, Functor.map_id, Comp.mk]

#check Functor.map_comp_map
-- (fun a a_1 => a <$> a_1) h ∘ (fun a a_1 => a <$> a_1) g' = (fun a a_1 => a <$> a_1) (h ∘ g')

-- protected theorem comp_map (g' : α → β) (h : β → γ) 
--     : ∀ (x : Comp F G α), Comp.map (h ∘ g') x = Comp.map h (Comp.map g' x)
--   | Comp.mk x => 
--   by
--     simp [Comp.map, Comp.map_mk, Functor.map_comp_map g' h, Functor];
...
instance : Seq (Comp F G) :=
  ⟨@fun _ _ f x => Comp.seq f (x Unit.unit)⟩
