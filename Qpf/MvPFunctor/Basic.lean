...
-/
def apply (α : TypeVec.{u} n) : Type u 
  := Σ a : P.A, P.B a ⟹ α
...
instance : MvFunctor.Lawful P.apply := 
...
@[simp]
theorem comp.get_mk (x : P.apply (λ i => (Q i).apply α)) : comp.get (comp.mk x) = x :=
begin
  cases x,
...
variable {α : TypeVec n} (p : ∀ i , α i → Prop) (x : P.apply α)
#check P.apply (λ i => Subtype (p i))
#reduce P.apply (λ i => Subtype (p i))
#reduce liftp p x
...
theorem liftp_iff {α : TypeVec n} (p : ∀ ⦃i⦄ , α i → Prop) (x : P.apply α) :
...
theorem liftr_iff {α : TypeVec n} (r : ∀ {i}, α i → α i → Prop) (x y : P.apply α) :
